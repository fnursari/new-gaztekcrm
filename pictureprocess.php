...
function permayap($deger) {
	...
	$clean = preg_replace("/[^a-zA-Z0-9\/_|+ -]/", '', $deger);
...
function getImageNameControl($name,$table,$id,$buyuk,$kucuk,$c=1)
{
	global $db;
	$isim_varmi=$db->get_var("select count($id) from $table where $kucuk='$name' or $buyuk='$name'");
	if($isim_varmi>0)
	{
		$cnt=$c+1;
		$name2= str_replace("-".$c.".", "-".$cnt.".", $name);
		return getImageNameControl($name2,$table,$id,$buyuk,$kucuk,$cnt);
	} else {
		return $name;
	}
}
