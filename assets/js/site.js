...
/*
	$(document).on('change','#para_birimi',function() {*/
		var suffixCurrency = $("#para_birimi").find('option:selected').attr('data-para-birimi');
		$('.price').inputmask({ 'alias': 'numeric', 'groupSeparator': ',' , 'autoGroup': true, 'digits': 2, 'digitsOptional': true, 'suffix': ' '+suffixCurrency });
/*	});*/
	...
	$(document).on('change','#satis_tipi,#makina_fiyati,#kompresor_fiyati,#fan_fiyati,#ups_fiyati,#kurutucu_fiyati',function() {
		...
		if(kdv_orani==0) {
			kdvsiz_toplam = makina_fiyati + kompresor_fiyati + fan_fiyati + ups_fiyati + kurutucu_fiyati; 
			genel_toplam =kdvsiz_toplam;
			console.log(kdvsiz_toplam); 
			$("#kdv").val(kdv);
			$("#genel_toplam").val(genel_toplam);
			$("#pesinat").val(genel_toplam/2);
		}
	...
	$(document).on('change','#plazma_kesim',function() {
		$("#kesim_kalinligi").val($("#plazma_kesim").find('option:selected').attr('data-kesim-kalinligi'));
	})


	$(document).on('change','#kesim_olcusu',function() {
		$("#tezgah_olcusu").val($("#kesim_olcusu").find('option:selected').attr('data-tezgah-olcusu'));
	})
